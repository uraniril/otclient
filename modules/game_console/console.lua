SpeakTypesSettings = {
    ...
    BoostedCreature = {
        speakType = MessageModes.BoostedCreature,
        color = '#FFFF00'
    }
}
...
function addTabText(text, speaktype, tab, creatureName)
    ...
    if speaktype.npcChat and
        ...
        if #highlightData > 0 then
            local labelHighlight = g_ui.createWidget('ConsolePhantomLabel',
                                                     label)
            ...
            for i = 1, #highlightData / 3 do
                ...
                highlightData[(i - 1) * 3 + 1] = dataBlock._start -
                                                     ((i - 1) * 2)
                highlightData[(i - 1) * 3 + 2] = dataBlock._end -
                                                     (1 + (i - 1) * 2)
            end
            label:setText(text)
            ...
            for i = 1, #highlightData / 3 do
                ...
                for letter = lastBlockEnd, dataBlock._start - 1 do
                    ...
                    local fillChar = (tmpChar == 10 or tmpChar == 32) and
                                         string.char(tmpChar) or
                                         string.char(127)

                    tmpText = tmpText ..
                                  string.rep(fillChar, letterWidth[tmpChar])
                end
                tmpText = tmpText .. dataBlock.words
            end

            -- Fill the highlight label to the same size as default label
            local finalBlockEnd = (highlightData[(#highlightData / 3 - 1) * 3 +
                                      2] or 1)
            for letter = finalBlockEnd, drawText:len() do
                local tmpChar = string.byte(drawText:sub(letter, letter))
                local fillChar = (tmpChar == 10 or tmpChar == 32) and
                                     string.char(tmpChar) or string.char(127)
...
function sendMessage(message, tab)
    ...
    if (channel or tab == defaultTab) and not chatCommandPrivateReady then
        ...
        g_game.talkChannel(SpeakTypesSettings[speaktypedesc].speakType, channel,
                           message)
        return
    else
...
function onTalk(name, level, mode, message, channelId, creaturePos)
    ...
    if (mode == MessageModes.Say or mode == MessageModes.Whisper or mode ==
        ...
        MessageModes.NpcFrom or mode == MessageModes.NpcFromStartBlock) and
        creaturePos then
        ...
        if isNpcMode then
            ...
            if #highlightData > 0 then
                for i = 1, #highlightData / 3 do
                    ...
                    staticMessage = staticMessage:gsub(
                                        "{" .. dataBlock.words .. "}",
                                        dataBlock.words)
                end
            end
